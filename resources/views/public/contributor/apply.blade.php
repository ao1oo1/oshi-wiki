            ...
            <div class="oshi-form-card" style="margin:0 auto;">
                <form method="POST" action="{{ route('public.contributor.apply.store') }}">
                    ...
                    <div class="mb-5 rounded bg-pink-50 p-4 text-sm">
                        <p>
                            申請日、登用開始日、登録作品件数、登録キャラクター件数はシステム側で保存・管理します。                        </p>                        <p class="mt-2">                            既にOshi-Wikiに登録済みのメールアドレスは使用できません。一般執筆ユーザーとして登録済みの場合は、別のメールアドレスでスタッフ申請してください。                        </p>                        <p class="mt-2">                            登用が開始された場合、入力したメールアドレス宛に管理スタッフ用ログインURLと仮パスワードを送信します。
                        </p>
                    </div>
